    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 overflow-x-hidden transition-colors duration-300">
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
          ...
          <StatCard label={`D1 Participants (${totals.day1Date || 'Day 1'})`} value={totals.day1Participants} total={registered} icon={<Coffee className="size-4 sm:size-5" />} color="emerald" />
          <StatCard label="D1 Total Scans" value={totals.day1TotalScans} icon={<Utensils className="size-4 sm:size-5" />} color="amber" />
          <StatCard label={`D2 Participants (${totals.day2Date || 'Day 2'})`} value={totals.day2Participants} total={registered} icon={<Coffee className="size-4 sm:size-5" />} color="indigo" />
          <StatCard label="D2 Total Scans" value={totals.day2TotalScans} icon={<Utensils className="size-4 sm:size-5" />} color="rose" />
        </div>
                  ...
                  <div key={i} className="px-8 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-mono italic">ID: {s.expoId || s.id || 'N/A'}</p>
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
              ...
              <p className="text-4xl font-black mb-2 tracking-tighter">
                {registered > 0 ? ((totals.day1Participants / registered) * 100).toFixed(1) : 0}%
              </p>
              <p className="text-xs font-medium opacity-80">Saturation of Day 1 distribution against total registered users.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2.5rem] shadow-xl">
              ...
              <div className="space-y-4">
                <MiniProgress label={`D2 Participants (${totals.day2Date})`} value={totals.day2Participants} total={registered} color="indigo" />
                <MiniProgress label="D2 Total Scans" value={totals.day2TotalScans} total={100} color="rose" />
              </div>
            </div>
          </div>
        </div>
